// Actualización para server.js - Implementar NewsAPI real
// Reemplazar la función de news existente

// Función real para NewsAPI
async function getNewsAPIData(query = 'stock market', pageSize = 20) {
  ...
  try {
    // Skip if demo key
    if (config.newsApi.apiKey === 'demo' || !config.newsApi.apiKey) {
    ...
    console.log(`Fetching real news from NewsAPI for: ${query}`);
    ...
    console.log(`NewsAPI returned ${articles.length} real articles`);
